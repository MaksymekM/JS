...
// Іншими словами : заповниои форму, натиснули кнопку, під формою з'явився блок з
// вашим об'єктом
// ==========================
let f = document.forms.f1;
    f.onsubmit = function (e) {
        ...
        h2.innerText =` First name:${f.name.value} Last name: ${f.surname.value} Age: ${f.age.value}`;
        div.appendChild(h2);
...
// =========================
//-->sessions.html and index.html created
...
function handler(page, limit=10){
    ...
    for (let i=startIndex; i<endIndex;i++) {
        ...
        title.innerText=`Tittle: ${location.title}`;
        type.innerText=`Type: ${location.type}`;

        item.append(title,type);
        newArr.push(item);
    }
    wrapperr.replaceChildren(...newArr);
